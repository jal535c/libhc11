...
#include "regfile.h"	//para acceder a su propia interfaz

//#include "memory.h"		//no es necesario, ya está incluido en regfile.h
						//por los tipos RegisterMap y RAM, son usados en INIT_C::Write
#include "ram.h"
...
{														//realmente todos los modos son iguales, solo cambia HPRIO, y no se usa.
														//si no paso parametro, usa SPECIAL_TEST como parametro por defecto
...
void RegisterFile::INIT_C::Write(byte_t val)	//tiene 2 atributos void*: regmap y chip_ram
{
	byte = val;
	...
	if ((temp = byte<<4) != 0)
	{
		...
		RegisterMap* pregmap = (RegisterMap*)regmap;	//regmap es el atributo, hace casting, ya que lo declaré como void*
														//tambien puedo declararlo como class RegisterMap* y quitar aqui casting
														//y en el constructor de HC11

		//RESUMEN: pregmap apunta a regmap (de INIT_C) que apunta a regmap (de HC11)
