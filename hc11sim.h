...
#include "hc11.h"	//necesito acceder a hc11.h (para HC11), memory.h (RAM, ROM), hc11misc (por log), basetypes.h (word_t, Word)
class RAM;
class ROM;
#include "wordtype.h"		//por los tipos word_t, Word
...
public:    
    ...
    HC11Sim();		
    ~HC11Sim();    
	void Config(HC11SimConfig ncfg);		//no se usa?, para añadir expansion de ram o rom para el robot, se llama en constructor Doc
    void SetupPseudoVectors(word_t vector_base_addr);	//se llama en Config
    void SetStartAddress(Word address);		//no se usa?, cambia la direccion en briefcase o rugwarrior, se llama en constructor Doc
