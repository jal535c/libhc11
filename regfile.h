...
//este fichero contiene la definicion de todos los registros y sus bits
...
//#include "hc11io.h"		//para acceder a PortRegister, PortARegister, PortBRegister, DDRegister
				//necesito acceder a los tipos bit_t (basetypes.h), 
				//ByteRegister y WordRegister (register.h)
				//en vez de llamar a los 2, llama a memory.h que ya los incluye
...
#include "bytetype.h"
#include "wordtype.h"
//#include "register.h"		//por los tipos ByteRegister y WordRegister
#include "byteRegister.h"
#include "wordRegister.h"


//#include "memory.h"		//en regfile.cpp (RegisterFile::INIT_C::Write(byte_t data)) necesito la clase RegisterMap, y la clase RAM 
...
public:		//todo es publico, las clases anidadas y los atributos de RegisterFile

	//las clases internas anidadas hacen referencia a registros y tienen como atributos a otros registros con los que tienen
	//relacion
	//Y todas tienen un metodo Write que redefine el de la clase padre
	//Ese metodo Write hace algo especial, de ahi la necesidad de estas clases anidadas
	//En el constructor de RegisterFile se vinculan los atributos de ella con los de las clases internas, osea:
	//tengo RegisterFile::PORTA y RegisterFile::CFORC_C::portA, y los tengo que vincular
	...
	public:
		...
		void* chip_ram;				//porque desde aqui necesito cambiar la configuracion de hc11
									//void* se usa para no tener que declarar el tipo RegisterMap* ya que todavia no está creado
									//como intermediario
	};
	...
	RegisterFile();
	void Reset(Mode mode= SPECIAL_TEST);	//esta funcion no está implementada, falta la palabra mode antes de =, aun así funciona?
...
*******************************************************************************/
	...
	inline bit_t CCF()	{ return SCSR.bit7(); }		//duda? sería return ADCTL.bit7();
