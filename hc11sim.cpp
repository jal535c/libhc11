#include "hc11sim.h"	//para acceder a su interfaz
...
void HC11Sim::Config(HC11SimConfig ncfg) {
    ...
    switch(cfg) {
		...
		case BriefCase:
		    ...
		    SetupPseudoVectors(0xFFC0);		//llama a la funcion k esta mas abajo en esta clase      
            break;
...
void HC11Sim::SetupPseudoVectors(word_t vector_base_addr) {			//se llama solo desde el metodo HC11Sim::Config
...
bool HC11Sim::LoadS19File(const char *fname) {		//carga un fichero .s19 en la memoria, le paso como parametro el path
    ...
    while (fgets(line, 256, fin) != NULL) {		//lee caracteres de fin y los mete en line, hasta 255 o CR, osea, lee una linea
    	...
    	if (line[0] == 'S') {
            ...
            switch(line[1]) {
                ...
                case '1':
					...
					cksm = bc + (addr & 0xFF) + ((addr >> 8) & 0xFF);   //suma la parte alta y baja de la direccion: desplaza y mascara (AND bit a bit)
					...
					cksm = ~cksm & 0xFF;	//complemento a 1 (invierte), mascara para coger solo la parte baja (AND bit a bit), invertir es como FF-cksm
